...
def test_eva_mink_ik():
    ...
    pos, rot = solver.fk(home_joints); rot = _as_rotation(rot)
    ...
    if solution is not None:
        ...
        # Verify with FK
        achieved_pos, achieved_rot = solver.fk(solution); achieved_rot = _as_rotation(achieved_rot)
...
def compare_solvers():
    ...
    pos_t, rot_t = trac_solver.fk(home_joints); rot_t = _as_rotation(rot_t)
    pos_m, rot_m = mink_solver.fk(home_joints); rot_m = _as_rotation(rot_m)

    fk_pos_err = np.linalg.norm(pos_t - pos_m)
    fk_rot_err = _rot_geodesic(rot_t, rot_m)
    print(f"   Trac FK @ home: pos={pos_t}, euler(deg)={rot_t.as_euler('xyz', degrees=True)}")
    print(f"   Mink FK @ home: pos={pos_m}, euler(deg)={rot_m.as_euler('xyz', degrees=True)}")
    ...
    if trac_solution is not None:
        pA, rA = trac_solver.fk(trac_solution); rA = _as_rotation(rA)
        pos_errA = np.linalg.norm(pA - target_pos)
        rot_errA = _rot_geodesic(rA, target_rot)
        print(f"   TracIK achieved pos: {pA} | pos_err={pos_errA:.6e} m | rot_err={rot_errA:.6e} rad")
    else:
        print("   TracIK failed on TRAC-based shared target")

    if mink_solution is not None:
        pB, rB = mink_solver.fk(mink_solution); rB = _as_rotation(rB)
        pos_errB = np.linalg.norm(pB - target_pos)
        rot_errB = _rot_geodesic(rB, target_rot)
        print(f"   Mink achieved pos:  {pB} | pos_err={pos_errB:.6e} m | rot_err={rot_errB:.6e} rad")
    else:
    ...
    if trac_solution is not None and mink_solution is not None:
        ...
        print(f"   Joint-space Δ between solvers (TRAC-based target) [L2]: {jdelta:.6e}")
    ...
    if trac_solution2 is not None:
        pA2, rA2 = trac_solver.fk(trac_solution2); rA2 = _as_rotation(rA2)
        pos_errA2 = np.linalg.norm(pA2 - target_pos2)
        rot_errA2 = _rot_geodesic(rA2, target_rot2)
        print(f"   TracIK achieved pos: {pA2} | pos_err={pos_errA2:.6e} m | rot_err={rot_errA2:.6e} rad")
    else:
        print("   TracIK failed on MINK-based shared target")

    if mink_solution2 is not None:
        pB2, rB2 = mink_solver.fk(mink_solution2); rB2 = _as_rotation(rB2)
        pos_errB2 = np.linalg.norm(pB2 - target_pos2)
        rot_errB2 = _rot_geodesic(rB2, target_rot2)
        print(f"   Mink achieved pos:  {pB2} | pos_err={pos_errB2:.6e} m | rot_err={rot_errB2:.6e} rad")
    else:
    ...
    if trac_solution2 is not None and mink_solution2 is not None:
        ...
        print(f"   Joint-space Δ between solvers (MINK-based target) [L2]: {jdelta2:.6e}")
    ...
    mink_on_trac = mink_solver.ik(trac_target_pos, _as_matrix(trac_target_rot), home_joints)
    if mink_on_trac is not None:
        p_m_on_t, r_m_on_t = mink_solver.fk(mink_on_trac); r_m_on_t = _as_rotation(r_m_on_t)
        pos_err_mt = np.linalg.norm(p_m_on_t - trac_target_pos)
        rot_err_mt = _rot_geodesic(r_m_on_t, trac_target_rot)
        print(f"      Mink→(Trac target) pos_err={pos_err_mt:.6e} m | rot_err={rot_err_mt:.6e} rad")
    else:
    ...
    trac_on_mink = trac_solver.ik(mink_target_pos, _as_matrix(mink_target_rot), home_joints)
    if trac_on_mink is not None:
        p_t_on_m, r_t_on_m = trac_solver.fk(trac_on_mink); r_t_on_m = _as_rotation(r_t_on_m)
        pos_err_tm = np.linalg.norm(p_t_on_m - mink_target_pos)
        rot_err_tm = _rot_geodesic(r_t_on_m, mink_target_rot)
        print(f"      Trac→(Mink target) pos_err={pos_err_tm:.6e} m | rot_err={rot_err_tm:.6e} rad")
    else:
    ...
    for name, tpos in extra_targets:
        ...
        if tA is not None:
            pA, rA = trac_solver.fk(tA); rA = _as_rotation(rA)
            eposA = np.linalg.norm(pA - tpos)
            erotA = _rot_geodesic(rA, rot_t)
            print(f"      Trac achieved pos_err={eposA:.6e} m | rot_err={erotA:.6e} rad")
        else:
            print("      Trac failed")

        if tB is not None:
            pB, rB = mink_solver.fk(tB); rB = _as_rotation(rB)
            eposB = np.linalg.norm(pB - tpos)
            erotB = _rot_geodesic(rB, rot_t)
            print(f"      Mink achieved pos_err={eposB:.6e} m | rot_err={erotB:.6e} rad")
        else:
            print("      Mink failed")
    ...
    R_off, t_off = estimate_and_validate_offset(
        trac_solver, mink_solver,
        pos_t, rot_t,  # TRAC FK at home
        pos_m, rot_m,  # MINK FK at home
        home_joints
    )
    ...
    print("=" * 60)
    print("- FK GAP: pure forward mismatch at identical joints; near-constant bias ⇒ frame/site/tool offset.")
    print("- IK GAP ([2] TRAC-based, [2b] MINK-based): how close each gets to the same Cartesian target; compare joint Δ too.")
    print("- CROSS-GAP ([3]): using one solver’s FK as target for the other isolates frame/orientation convention differences.")
    print("- [4] and [4b]: directional consistency checks around each solver’s native FK pose.")
    print("- [5],[6]: estimate constant SE(3) and diagnose residual variance to find which joints/links cause the gap.")
...
def _estimate_se3_offset(trac_solver, mink_solver, n_samples=60, rng_seed=0):
    ...
    for q in Q:
        pT, RT = trac_solver.fk(q); RT = _as_rotation(RT)
        pM, RM = mink_solver.fk(q); RM = _as_rotation(RM)
    ...
    for i, q in enumerate(Q):
        # true RM:
        _, RM_true = mink_solver.fk(q); RM_true = _as_rotation(RM_true)
    ...
    stats = {
        "pre_pos_mean":  pre_pos.mean(),   "pre_pos_med":  np.median(pre_pos),
        "post_pos_mean": post_pos.mean(),  "post_pos_med": np.median(post_pos),
        "pre_rot_mean":  pre_rot.mean(),   "pre_rot_med":  np.median(pre_rot),
        "post_rot_mean": post_rot.mean(),  "post_rot_med": np.median(post_rot),
    }
...
def estimate_and_validate_offset(trac_solver, mink_solver, pos_t, rot_t, pos_m, rot_m, home_joints):
    """
    Convenience wrapper:
      - estimates T_off
    ...
    R_off, t_off, stats = _estimate_se3_offset(trac_solver, mink_solver, n_samples=80, rng_seed=1)

    print("   Pre/Post residuals over random FK samples:")
    print(f"      pos  mean (pre→post): {stats['pre_pos_mean']:.6f} → {stats['post_pos_mean']:.6f} m")
    print(f"      pos median (pre→post): {stats['pre_pos_med']:.6f} → {stats['post_pos_med']:.6f} m")
    print(f"      rot  mean (pre→post): {stats['pre_rot_mean']:.6f} → {stats['post_rot_mean']:.6f} rad")
    print(f"      rot median (pre→post): {stats['pre_rot_med']:.6f} → {stats['post_rot_med']:.6f} rad")
    ...
    target_rot_T = rot_t
    targM_pos, targM_rot = _apply_trac_to_mink_target(target_pos_T, target_rot_T, R_off, t_off)
    mink_sol_adj = mink_solver.ik(targM_pos, _as_matrix(targM_rot), home_joints)
    if mink_sol_adj is not None:
        pM_hit, RM_hit = mink_solver.fk(mink_sol_adj); RM_hit = _as_rotation(RM_hit)
        print(f"      Mink (transformed target) pos_err={np.linalg.norm(pM_hit - targM_pos):.6e} m | rot_err={_rot_geodesic(RM_hit, targM_rot):.6e} rad")
    else:
    ...
    target_rot_M = rot_m
    targT_pos, targT_rot = _apply_mink_to_trac_target(target_pos_M, target_rot_M, R_off, t_off)
    trac_sol_adj = trac_solver.ik(targT_pos, _as_matrix(targT_rot), home_joints)
    if trac_sol_adj is not None:
        pT_hit, RT_hit = trac_solver.fk(trac_sol_adj); RT_hit = _as_rotation(RT_hit)
        print(f"      Trac (transformed target) pos_err={np.linalg.norm(pT_hit - targT_pos):.6e} m | rot_err={_rot_geodesic(RT_hit, targT_rot):.6e} rad")
    else:
...
def _collect_fk_pairs(trac_solver, mink_solver, Q):
    ...
    pT, pM, RT, RM = [], [], [], []
    for q in Q:
        p_t, r_t = trac_solver.fk(q); r_t = _as_rotation(r_t)
        p_m, r_m = mink_solver.fk(q); r_m = _as_rotation(r_m)
        pT.append(np.asarray(p_t, float)); RT.append(r_t)
        pM.append(np.asarray(p_m, float)); RM.append(r_m)
...
def _numeric_jacobian_fk_pos(solver, q, eps=1e-6):
    ...
    for j in range(dof):
        ...
        p1, _ = solver.fk(q + dq); p1 = np.asarray(p1, float)
        J[:, j] = (p1 - p0) / eps
    return J
...
def _numeric_jacobian_fk_rotvec(solver, q, eps=1e-6):
    ...
    dof = q.size
    J = np.zeros((3, dof))
    _, R0 = solver.fk(q); R0 = _as_rotation(R0)
    for j in range(dof):
        dq = np.zeros_like(q); dq[j] = eps
        _, R1 = solver.fk(q + dq); R1 = _as_rotation(R1)
        dR = R0.inv() * R1
...
def _jacobian_gap_stats(trac_solver, mink_solver, Q, report_k=5):
    ...
    print("\n[Diag] Mean ||ΔJ_pos|| per joint:", "  ".join(f"j{j+1}:{v:.4f}" for j, v in enumerate(pos_mean)))
    print("[Diag] Mean ||ΔJ_rot|| per joint:", "  ".join(f"j{j+1}:{v:.4f}" for j, v in enumerate(rot_mean)))

    # Biggest gaps
    jp = np.argsort(-pos_mean)[:report_k]; jr = np.argsort(-rot_mean)[:report_k]
    print(f"[Diag] Top-{report_k} J_pos gaps at joints:", [int(i+1) for i in jp], "values:", [float(pos_mean[i]) for i in jp])
    print(f"[Diag] Top-{report_k} J_rot gaps at joints:", [int(i+1) for i in jr], "values:", [float(rot_mean[i]) for i in jr])
...
def _fit_joint_zero_offsets_on_rot(trac_solver, mink_solver, Q):
    ...
    Rs = []; Jall = []
    for q in Q:
        _, RT = trac_solver.fk(q); RT = _as_rotation(RT)
        _, RM = mink_solver.fk(q); RM = _as_rotation(RM)
        Rrel = (RT.inv() * RM).as_rotvec()    # desired ~ J * delta_q
        Jt_r = _numeric_jacobian_fk_rotvec(trac_solver, q)  # (3,dof)
        Rs.append(Rrel); Jall.append(Jt_r)
...
def run_gap_diagnostics(trac_solver, mink_solver, R_off, t_off, seed=2):
    ...
    # Pre:
    dP_pre = pM - pT
    dR_pre = np.array([(RT[i].inv() * RM[i]).as_rotvec() for i in range(len(Q))])  # (N,3)
    # Post (apply Trac→Mink offset):
    pT_comp = R_off.apply(pT) + t_off
    dP_post = pM - pT_comp
    dR_post = np.array([((R_off * RT[i]).inv() * RM[i]).as_rotvec() for i in range(len(Q))])

    # Variance & PCA (position part)
    pre_var = dP_pre.var(axis=0); post_var = dP_post.var(axis=0)
    ...
    print("      post:", [f"{wi:.4f}" for wi in w_post[:-1]], "bias", f"{w_post[-1]:.4f}")
